...
export async function renderChapter(book:Book,index:number){
  ...
  const blocks:string[]=[];for(const node of Array.from(doc.querySelectorAll('h1,h2,h3,h4,p,li,blockquote')).filter(n=>!n.querySelector('p,li,blockquote'))){const value=node.textContent?.trim()||'';if(!value)continue;node.setAttribute('data-block',String(blocks.length));blocks.push(value)}
